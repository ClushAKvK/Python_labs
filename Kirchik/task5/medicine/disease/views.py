...
from . import managedb, xml_disease
from django.apps import apps

# Create your views here.
models = apps.get_app_config('disease').get_models()
...
def index(request):
    ...
    return render(request, 'disease/index.html', content)


def create_xml(request):
    xml_disease.create_xml(apps.get_app_config('disease').get_models())
    return redirect(reverse('disease:index'))
...
def detail(request, table_id):
    ...
    for record in records:
        for key, val in record.items():
            if model._meta.model_name == 'diseasesymptom':
                if key == 'disease_id':
                    record[key] = Disease.objects.get(pk=int(val)).name
                elif key == 'symptom_id':
                    record[key] = Symptom.objects.get(pk=int(val)).name
    ...
    return render(request, 'disease/detail.html', content)
...
def add_record(request, table_id):
    ...
    if model._meta.model_name == 'diseasesymptom':
        params.append(request.POST['disease'])
        params.append(request.POST['symptom'])
    else:
    ...
    return redirect(reverse('disease:detail', args=(table_id,)))
